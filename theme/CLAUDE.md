...
and the three roles that differ say why beside their value.
...
4. Write `dotfiles/<pkg>/theme-emit`. It takes `--list` and prints the files it owns, or takes
   ...
   files with `Generated by theme-emit` in the first three lines. Exit 2 if it cannot run on
   this machine, any other nonzero if what it was given is wrong. Ghostty's is the settled
   example for a file of `key = value` lines, Neovim's for a file that is code.
...
`lua/aura/palette.lua`. Everything else still carries its own copy of these values and is
listed in `decisions/theme-palette.md` with what each one needs, herdr first since it has no
include and its two theme tables have to be rewritten in place.
