...
#
# It is not a theoretical gap. A hook line added to this template once reached no existing
# machine, so the shell that was supposed to run it simply did not, and the only way to land
# it was to move .zshrc aside by hand.
#
...
GENERATED="$(cat << EOF
# PATH, hoisted above everything else. macOS does not put the Homebrew prefix on the PATH it
# hands a login shell, so nothing here may assume it, and ~/.local/bin comes first so a binary
# a person puts there wins over any package manager copy still lying around.
