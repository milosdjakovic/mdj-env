...
while IFS= read -r emit; do
    ...
    [[ -f "$pkg_dir/theme-exempt" ]] && err "$pkg has a theme-emit and a theme-exempt, and a package painted from the palette cannot also say it is not"
...
# A hex colour anywhere under dotfiles is a second owner of a value the palette already
# holds, which is the drift this whole layer exists to end. Every file is read, not only the
# maps and the emitters, because a hand copy sits in a tool's own config or a script, never
# in a map. The generated files carry hex on purpose and are exempt by their banner, which
# also exempts the hand written part of a file an emitter owns only in part. A file is
# reported once, with how many it carries and where the first one is, since a whole hand
# written theme is one finding rather than thirty.
#
# Two declarations exempt a file, both found by name and neither a list kept here. A package
# that is not painted from the palette says so once, in a theme-exempt file at its root beside
# its DEPENDENCIES, which is where a package already speaks for itself, and the file's whole
# content is the reason. A single file whose colours belong to someone else, brand colours or
# a palette for another medium, carries a line reading theme-exempt, a comma, and the reason.
# Every exemption is printed on every run with its reason so none leaves view, and one with
# no reason is an error, since an exemption with no reason is a hex value with no owner.
say "==> Hex values outside the palette"
leaks=0
exempted=0
declared_pkgs=" "
while IFS=$'\t' read -r path n first; do
    rel="${path#"$ROOT"/}"
    pkg="${rel#dotfiles/}"; pkg="${pkg%%/*}"
    if [[ "$rel" == dotfiles/* && -f "$DOTFILES/$pkg/theme-exempt" ]]; then
        [[ "$declared_pkgs" == *" $pkg "* ]] && continue
        declared_pkgs="$declared_pkgs$pkg "
        reason="$(tr -s '[:space:]' ' ' <"$DOTFILES/$pkg/theme-exempt" | sed -e 's/^ //' -e 's/ $//')"
        if [[ -z "$reason" ]]; then
            err "$pkg/theme-exempt gives no reason, and an exemption with no reason is a hex value with no owner"
        else
            say "  exempt $pkg, $reason"
            exempted=$((exempted + 1))
        fi
        continue
    fi
    # The bracket expressions below name the POSIX space class rather than a backslash t,
    # because BSD sed reads that pair inside brackets as two literal characters and stripped
    # the t of every reason ending in one.
    if grep -q 'theme-exempt' "$path"; then
        reason="$(grep -m1 'theme-exempt' "$path" | sed -e 's/^.*theme-exempt[[:space:]]*,\{0,1\}[[:space:]]*//' -e 's/[[:space:]]*-->[[:space:]]*$//' -e 's/[[:space:]]*\*\/[[:space:]]*$//' -e 's/[[:space:]]*$//')"
        if [[ -z "$reason" ]]; then
            err "$rel carries $n hex value(s) and a theme-exempt line with no reason, and an exemption with no reason is a hex value with no owner"
        else
            say "  exempt $rel, $reason"
            exempted=$((exempted + 1))
        fi
        continue
    fi
    err "$rel carries $n hex value(s), the first at line $first, and only theme/palettes may"
    leaks=$((leaks + 1))
done < <(grep -rnIE '#[0-9a-fA-F]{6}\b' "$DOTFILES" "$THEME/active.toml" "$VOCABULARY" 2>/dev/null \
    | awk -F: '{ if (!($1 in n)) { order[++k] = $1; first[$1] = $2 } n[$1]++ } END { for (i = 1; i <= k; i++) printf("%s\t%d\t%d\n", order[i], n[order[i]], first[order[i]]) }' \
    | while IFS=$'\t' read -r path n first; do grep -q "Generated by theme-emit" "$path" || printf '%s\t%s\t%s\n' "$path" "$n" "$first"; done \
    | sort)
if [[ $leaks -eq 0 ]]; then
    if [[ $exempted -eq 0 ]]; then say "  none"; else say "  none beyond the $exempted declared above"; fi
fi
