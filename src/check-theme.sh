...
resolve_half() {
    ...
    while read -r role policy _; do
        ...
        if [[ "$raw" == \{* ]]; then
            if [[ -n "$(table_field "$raw" color)" ]]; then
                ...
                hex="$(blend "$top" "$under" "$amount")"
            elif [[ -n "$(table_field "$raw" darken)" || -n "$(table_field "$raw" lighten)" ]]; then
                colour="$(table_field "$raw" color)"
...
while IFS= read -r emit; do
    ...
    if [[ $status -eq $CANNOT_RUN_HERE ]]; then
        ...
        while IFS= read -r f; do
            ...
            if ! head -n 3 "$pkg_dir/$f" | grep -q "Generated by theme-emit"; then
                err "$pkg, $f does not open with the generated banner, so nothing warns a person off editing it"
            fi
        done <<<"$owned"
    fi
...
[[ $stale -eq 0 && $unchecked -eq 0 && $count -gt 0 ]] && say "  every generated theme is current, $count tool(s)"
