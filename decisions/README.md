...
- [homebrew-upgrades](homebrew-upgrades.md), the Brewfile step installs what is missing, upgrades nothing and never adopts an app it did not install, since a version bump is a deliberate act rather than something a setup run does on the way past
...
- [dev-defaults](dev-defaults.md), the default editor step asks which editor rather than assuming one, reads each handler back rather than trusting duti, since on macOS 27 a change is a dialog the person answers, and says one line when nothing moved
