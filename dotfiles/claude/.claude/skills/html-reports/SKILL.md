...
- **There is no h4 or below.** The small uppercase labels that head a subsection, such as the size line or the who it helps list, are not headings. They are an `.eyebrow` label, a bold uppercase sans line in the muted tone. Using a heading tag for them would break the scale, so they are always a styled label instead.
...
The first card is always an overview that frames the document, and each following section is its own card. Section cards lead with the serif title, then a small meta row directly beneath it holding any reference link and a size or status badge.
...
- **Choices list.** A compact list with no dividers, each row a short bold fork the reader decides followed by what each way costs or simplifies. Use it when the size or shape of the thing genuinely depends on the reader's own answers, so scope reads as a conversation rather than one fixed number. Keep it visually lighter than the value list so the two do not blur together.
- **Estimate line.** A single prominent figure under the lead. The uppercase sans label sits on its own line, and the value follows on the line below in bold serif, matching the label then content rhythm of every other block. No divider or rule above it. Use it for the one number a section should surface, such as a build estimate.
