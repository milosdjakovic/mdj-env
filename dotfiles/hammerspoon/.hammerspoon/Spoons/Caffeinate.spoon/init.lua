...
--- (the keep awake mechanism) and the view (the webview mechanism), names them, and
--- wires them together. The view draws the rows, the inline fields, and the status,
--- and hands back the applied row with its hours and minutes. This file turns that
--- into an engine call, and returns an error string for the view to show when a
--- value is missing. It is module style with plain functions, so it exposes the same
--- small control surface the clipboard manager does (isShowing, hide), which the
--- shared control in the main root calls on whichever tool is open.
---
--- Why a webview and not the shared Chooser atom. The chooser is a list picker whose
--- rows are always numbered and its field cannot constrain input, right for the
--- clipboard but wrong here. Keep awake wants a short list with inline numeric fields
--- that clamp to hours and minutes, which a webview gives natively (webview.lua). It
--- can also take focus, because keep awake pastes nothing back, unlike the clipboard.

local M = { name = "Caffeinate", version = "2.0", author = "mdj-env" }

-- Load the engine and view siblings by absolute path, the Capture idiom (a spoon
-- dir is not on package.path).
local spoonPath = debug.getinfo(1, "S").source:sub(2):match("(.*/)")
local engine = dofile(spoonPath .. "engine.lua")
local viewModule = dofile(spoonPath .. "webview.lua")

local cfg = nil   -- injected: the shared theme
local view = nil
...
--- M.configure(opts) - inject the shared theme.
