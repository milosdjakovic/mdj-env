...
--- policy over the store and the shared insertion engine rather than new mechanism, and both
--- end on the same signal, a genuine copy. That shared reset is what earns them one file
--- instead of two. A third behaviour would be the point to split them.
---
...
function M.appendCopy()
  ...
  log.df("%.3f append press, reading=%s walking=%s", clock(), tostring(paste.isReading()), tostring(walking))
  if paste.isReading() or walking then
    return
  end
  ...
  local direct = paste.readSelection()
  if direct then
    accept(direct)
  else
    paste.copySelection(accept)
  end
end
...
function M.pasteNext()
  ...
  if runStillApplies() then
    ...
    run = {
      ...
      index = 1,
      app = frontmostID(),
      clipboard = paste.snapshotClipboard(),
    }
  ...
  if (entry.kind == "text" or entry.kind == "url") and paste.insertText(entry.text) then
  ...
  -- Nothing floats to the top after this paste, which is what lets the walk read the list
  -- without rewriting the order it is reading. The engine has no reorder option to pass any
  -- more, since what a paste means for a list is the caller's business, so a walk simply does
  -- not do it, the way a caller pasting out of the picker simply does.
  local started = paste.paste(entry, {
...
--- M.configure(opts) - inject the store, the insertion engine, the readers, util, media (for the
