...
stays in the hammerspoon `CLAUDE.md`, which links here. `HyperKey.spoon` and
`WindowLeader.spoon` are the adapters over this engine.
...
Today that is Caps Lock and Right Option, with Right Command defined but not
registered. Keeping the mechanism here and the meaning in the adapters is why
adding a leader is cheap and why the two adapters can share one resolver.
...
the clipboard's synthesized paste keeps these from looping back through the tap.
...
like the clipboard would open and close it over and over while the key stays down.
...
carry it. This is how a held nav key like a chooser's j or k scrolls continuously,
